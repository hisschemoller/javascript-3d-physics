...
function createMaskBall(){
  ...
  //threeJS Section
  let ball = new Mesh(new SphereBufferGeometry(radius), new MeshPhongMaterial({color: 0x00ff08}));
  ball.position.set(pos.x, pos.y, pos.z);
  ball.castShadow = true;
  ball.receiveShadow = true;
  scene.add(ball);
...
function createJointObjects() {
  ...
  //Sphere Graphics
  let ball = new Mesh(new SphereBufferGeometry(radius), new MeshPhongMaterial({color: 0xb846db}));
  ...
  //Block Graphics
  let block = new Mesh(new BoxBufferGeometry(), new MeshPhongMaterial({color: 0xf78a1d}));
